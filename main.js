...
function calcularcuatroPorMil() {
  ...
  const result = `El valor que debera pagar adicional por utilizar ${aMoneda(
    value,
    opcionesPesosColombianos
  )} pesos es: \n ${aMoneda(
    cuatroPorMil(value),
    opcionesPesosColombianos
  )} pesos.`;
  const total = `Total ( ${aMoneda(
    value,
    opcionesPesosColombianos
  )} + ${aMoneda(
    cuatroPorMil(value),
...
const calcPrecioFinal = (precio, porcentaje, cantidad) =>
  ((precio / (100 - porcentaje)) * 100) / cantidad;
...
function calcularPrecio() {
  ...
  const result = `El valor final de su producto es: \n ${aMoneda(
    calcPrecioFinal(price, porcent, cant),
    opcionesPesosColombianos
  )} pesos.`;
